...
const index = () => {
  ...
  const [sortCriteria, setSortCriteria] = useState("Most Upvotes");
  ...
  const upvote = async (suggestionId: String) => {
    // const upvote = e.target.title.value;

    const response = await fetch(
      `http://localhost:8000/Suggestion/${suggestionId}`,
      {
        method: "PATCH",
        headers: {
  ...
  if (sortCriteria === "Most Upvotes") {
    sortedSuggestions.sort((a, b) => b.upvotes - a.upvotes);
  } else if (sortCriteria === "Least Upvotes") {
  ...
  } else if (sortCriteria === "Least Comments") {
  ...
  if (category === "all") {
    ...
    filtered = sortedSuggestions.filter((item) => item.category === category);
  }
  ...
  return (
    <div className="bg-[#F7F8FD] absolute inset-0 w-full h-max -z-10">
      <div className="flex lg:flex-row md:flex-col md:mx-[24px] md:ms-[39px] lg:mx-[165px] md:my-[94px] gap-[30px]">
        {/* Left */}
        <div className="lg:gap-6 lg:flex  md:grid md:grid-cols-3 md:gap-[10px] hidden lg:flex-col ">
          <div
            className={`lg:w-[255px] rounded-[10px] lg:h-[137px] md:w-[223px] md:h-[178px] sm:flex-none z-10 ${styles.background}`}
          >
            <div className={`lg:pt-[62px] md:pt-[103px] pl-[24px]`}>
              <h2 className={`text-[#fff] `}>Eqaim</h2>
              <p className={`text_md`} style={{ color: "#fff" }}>
                Feedback Board{" "}
              </p>
            </div>
          </div>
          <div className="lg:w-[255px] flex flex-wrap gap-2 bg-white md:w-[223px] p-6 rounded-[10px] z-10">
            <button
              className="btn py-1 px-4 h-[30px]"
          ...
          <div className="w-full gap-6 bg-white p-6 md:w-[223px] lg:w-full flex flex-col rounded-[10px] z-10">
            <div className="flex justify-between w-full">
              <h3>Roadmap</h3>
              <Link
                href="/Roadmap"
                className="underline text_sm_bold"
                style={{ color: "#4661E6" }}
              >
                View
              </Link>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <p className="text_lg">Planned</p>
                </div>
                <p className="text_sm_bold">{planningStatusCount}</p>
              </div>
              ...
              <div className="flex justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <p className="text_lg">In-Progress</p>
                </div>
                <p className="text_sm_bold">{progressStatusCount}</p>
              </div>
              ...
              <div className="flex justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <p className="text_lg">Live</p>
                </div>
                <p className="text_sm_bold">{liveStatusCount}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Right */}
        <div className="flex relative flex-col w-full gap-6">
          <div
            className={`md:hidden relative flex w-full h-[72px] -mb-6  ${styles.background}`}
          >
            <div className="flex justify-between py-4 w-full px-6">
              <div className="flex flex-col ">
                <h4 className={`text-[#fff] `}>Eqaim</h4>
                <p className={`text_sm`} style={{ color: "#fff" }}>
                  Feedback Board{" "}
                </p>
              </div>
              {!menu && (
          ...
          {menu && (
            <div
              className={`absolute w-3/4 h-screen right-0 top-[72px] bg-[#F7F8FD] p-6 ${styles.shadow}`}
            >
              <div className="lg:w-[255px] flex flex-wrap gap-2 bg-white md:w-[223px] p-6 rounded-[10px] z-10">
                <button
                  className="btn py-1 px-4 h-[30px]"
          ...
          <div className="bg-[#373F68] md:rounded-[10px] h-[72px] flex items-center justify-between  p-6">
            <div className="flex">
              <div className="md:flex items-center hidden justify-between ">
                ...
                <h3 className={`text-[#fff] pl-4 pr-[38px]`}>
                  {filtered?.length} Suggestions
                </h3>
              </div>
              <div className="flex  text-[#fff] items-center">
                <button
                  className="flex items-center gap-1 font-light sort_btn"
                  onClick={handelActive}
                >
                  Sort by : <h4 className="text-[#fff]">{sortCriteria}</h4>
                ...
                {active && (
                  <div
                    className={`${styles.shadow} absolute top-20 left-1/4 w-[255px]  px-6 py-3  h-[192px]  text_lg rounded-[10px] bg-white`}
                    style={{ color: "#647196" }}
                  >
                    <button
                      className={`${styles.hover} border-b w-full text-start pb-3`}
                      onClick={(e) => {
                        setSortCriteria("Most Upvotes");
                      }}
                    >
                      Most Upvotes
                    </button>
                    <button
                      className={`${styles.hover} border-b w-full text-start py-3`}
                      onClick={(e) => {
                        setSortCriteria("Least Upvotes");
                      }}
                    >
                      Least Upvotes
                    </button>
                    <button
                      className={`${styles.hover} border-b w-full text-start py-3`}
                      onClick={(e) => {
                        setSortCriteria("Most Comments");
                      }}
                    >
                      Most Comments
                    </button>
                    <button
                      className={`${styles.hover}  w-full text-start py-3`}
                      onClick={(e) => {
                        setSortCriteria("Least Comments");
                      }}
                    >
                      Least Comments
                    </button>
            ...
            <div>
              <Link
                href="/NewFeedback"
                className={`button1 md:w-[158px] w-[134px] h-[44px] rounded-[10px]`}
              >
                {" "}
                + Add Feedback
              </Link>
          ...
          <div className=" flex flex-col items-center justify-center rounded-[10px] w-full ">
            {filtered?.length === 0 && (
              <div className=" flex bg-white flex-col justify-center items-center w-full h-screen">
                <Image
                  ...
                  height={137}
                  objectFit="cover"
                  className="absolute rounded-[10px] inset-0"
                />
                <div className="flex flex-col gap-4 pt-14 pb-12 justify-center items-center">
                  {" "}
                  <h1>There is no Feedback</h1>
                  <p
                    className="text_lg max-w-[410px] text-center"
                    style={{ color: "#647196" }}
                  >
                ...
                <Link
                  href="/NewFeedback"
                  className="button1 md:w-[158px] sm:w-[134px] h-[44px]"
                >
                  + Add Feedback
                </Link>
              </div>
            ...
            {filtered?.length != 0 && (
              <div className="flex flex-col w-full md:px-0 px-6 gap-5">
                {filtered?.map((item) => (
                  <div className="flex bg-white rounded-[10px] items-center justify-between w-full container px-8 py-7 h-[151px]">
